...
def exclude_empty_text_bboxes(bboxes):
    ...
    for _bbox in bboxes:
        if _bbox.txt.strip() == "":
            ...
            exclued_boxes.append(_bbox)
            bboxes.remove(_bbox)
    logger.debug("原bboxes[%d]个，删除了[%d]个空字符串的，还剩[%d]个",
                 original_size,
                 len(exclued_boxes),
                 len(bboxes))
    return exclued_boxes
...
def exclude_boxes_by_statistics(bboxes, sigma_num=2):
    ...
    return good_bboxes, bad_bboxes, mean


# 剔除一个标准差之外的数据
# raw_data,原始数据[N,4,2] : [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]
def exclude_1sigma(raw_data, sigma_num=2):
    ...
    bad_heights_indices = np.where(np.logical_or(heights > max_height, \
                                                 heights < min_height, \
                                                 heights > 2 * mean))
